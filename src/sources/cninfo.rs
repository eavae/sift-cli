//! cninfo `szse_stock.json` + `hke_stock.json` row types and JSON
//! envelope parser.
//!
//! Field semantics are pinned in the F1 README "数据源与协议"; in
//! particular `category` is kept as a raw string (`"A股"` / `"B股"` /
//! `"CDR"` / `"港股"`) so that an upstream addition does not break
//! deserialization.
